...
public class PictureTransformer : MonoBehaviour
{
    ...
    public void SaveThisTexture2D()
    {
        if (RP)
        {
            Texture2D pic = new Texture2D(1111, 1111);
            byte[] data = System.Convert.FromBase64String(ToImageString);
            pic.LoadImage(data);
            byte[] bytes = pic.EncodeToPNG();
    ...
    public void ReloadPicture()
    {
        ...
        {
            ToImageString = ChatGuiAddon.PictureBytes[PictureNumber-1];
    ...
    void Start()
    {
        ...
        for(int I = 0; I < AImages.Length; I++)
        {
            ...
            if (AImages[I].GetComponent<PictureTransformer>().PictureNumber >= PictureNumber)
            {
                PictureNumber = AImages[I].GetComponent<PictureTransformer>().PictureNumber + 1;
            }
        }
        if (AImages.Length == 1)
        {
